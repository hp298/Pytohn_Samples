...
class Ship(GImage):
    ...
    def collides(self,bolt):
        """
        Checks if a bolt collides with ship

        parameter bolt: Bolt object instance
        precondition: bolt is a Bolt object
        """
        tl = [bolt.x - (BOLT_WIDTH / 2), bolt.y + (BOLT_HEIGHT / 2)]
        tr = [bolt.x + (BOLT_WIDTH / 2), bolt.y + (BOLT_HEIGHT / 2)]
        bl = [bolt.x - (BOLT_WIDTH / 2), bolt.y - (BOLT_HEIGHT / 2)]
        br = [bolt.x + (BOLT_WIDTH / 2), bolt.y - (BOLT_HEIGHT / 2)]
        if bolt.isPlayerBolt() == False and (self.contains(tl) or
        self.contains(tr) or self.contains(bl) or self.contains(br)):
            return True
        return False
...
class Alien(GImage):
    ...
    def collides(self,bolt):
        """
        Checks if a bolt collides with this alien
        ...
        bl = [bolt.x - (BOLT_WIDTH / 2), bolt.y - (BOLT_HEIGHT / 2)]
        br = [bolt.x + (BOLT_WIDTH / 2), bolt.y - (BOLT_HEIGHT / 2)]
        if bolt.isPlayerBolt() == True and (self.contains(tl) == True or
        self.contains(tr) == True or self.contains(bl) == True or
        self.contains(br) == True):
            return True
        return False
...
class Bolt(GRectangle):
    ...
    def isPlayerBolt(self):
        """
        returns whether this instance of bolt was fired by the ship
        """
        if self._velocity > 0:
            return True
        return False
