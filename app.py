...
class Invaders(GameApp):
    ...
    _speed: alien speed multiplier [0 < float <= 1]
    _score: label of the score [GLabel]
    _playerscore: Takes score from wave to use in the next wave [0 < int]
    """
    ...
    def start(self):
        ...
        if self._state == STATE_INACTIVE:
            self._text = GLabel(x = XSTART, y = YSTART,
            font_size = FONTSIZESTART, text = TEXTSTART)
            self._wave = None
        else:
            self._wave = None #Wave
            self._text = None
        self._speed = 1
        self._score = GLabel(x = SCORE_WIDTH / 2,
         y = GAME_HEIGHT - (SCORE_HEIGHT / 2), text = 'SCORE: 0')
        self._playerscore = 0
    ...
    def states(self, dt):
        """
        Impliments actions to the game dependent on the state of the game.
        ...
        if self._state == STATE_INACTIVE:
            ...
            self._wave = Wave()
            self._wave.setSpeedMult(self._speed)
            self._text = None
            self._wave.setScore(self._playerscore)
            self._state = STATE_ACTIVE
        elif self._state == STATE_ACTIVE:
            Wave.updateWave(self._wave, self.input, dt)
            self._score.text = 'Score: ' + str(self._wave.getScore())
            if self._wave.getShip() == None:
                if self._wave.getLives() != 0:
                    self._state = STATE_PAUSED
                    self._text = GLabel(x = XSTART, y = YSTART,font_size = FONTSIZESTART,
                    text = 'U ded, Press "s" to continue...')
                else:
                    self._state = STATE_COMPLETE
                    self._text = GLabel(x = XSTART, y = YSTART,font_size = FONTSIZESTART,
                    text = 'U suk, The End. Your score: ' + str(self._wave.getScore()))
            if self._wave.aliWin() == True:
                self._state = STATE_COMPLETE
                self._text = GLabel(x = XSTART, y = YSTART, font_size = FONTSIZESTART,
                text = 'U suk, The End. Your score: ' + str(self._wave.getScore()))
            if self._wave.playerWin():
                self._state = STATE_NEWWAVE
                self._speed = self._speed / 1.5
                self._playerscore = self._wave.getScore()
                #self._text = GLabel(x = XSTART, y = YSTART, font_size = FONTSIZESTART, text = 'WINNER! U DA PERSAN')
    ...
    def draw(self):
        ...
        if self._wave != None:
            Wave.wave_draw(self._wave, self.view)
        if self._score != None:
